...
把材料放到对话或仓库里，按**当前要做的那一步**说即可。Agent 先问未确认的问题，再出确认卡；你对卡上范围发 `go` 才会改文件。「好的 / 同意 / 可以」不算授权。

**从一份 PRD 开始**

| 你说 | Agent 会做 |
|---|---|
| 「按这份 PRD 写需求规格」 | 只转需求规格（docx / pdf / md / 粘贴正文均可） |
| 「按这份 PRD 做大屏」 | 仍先转规格；HTML 委托 `bi-dashboard-generator`；Vue / Spring 留在本 Skill |

不要指望一句「梳理需求、定计划并开发」同一轮全部落盘。

**规格已经审过**

| 你说 | Agent 会做 |
|---|---|
| 「下一步」 | 按写死顺序出**下一份**文档（选型 → 模型 → 契约 → 计划），不会直接写代码 |
| 「go 执行 M1–M3（页面或 mock 自测）」 | 仅在开发计划已审核后，按卡上模块范围连续实现 |

**每一轮你要做的**

1. 回答歧义问题（未答的不会写成确定需求）
2. 看确认卡，范围内发 `go`
3. 看 `review/`，接受则说「下一步」，否则指出要改的地方

项目已有总规则（如 `AI工作规范.md`）则协作门禁用项目的；没有时用 `SKILL.md` 默认（没有 `go` 不改文件）。
...
| 时间 | 说明 |
|---|---|
| 2026-09-03 | README：材料入口、转换门禁、使用方式按「你说 / Agent 会做」对照 |
