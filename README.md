...
本份歧义清单（未确认先问）→ 用户回答
  ...
  →（实现 go：须计划已审核；卡上写 Mx–My 与完成口径）范围内按依赖连续实现
  → 对照该口径自测通过后标「已完成」
```

需求规格、选型、数据模型、设计契约、开发计划不能一起出。同一轮 `go` 不得既写规格又写代码。无已审核开发计划不得写 Vue / Spring / 转 Vue。写需求 / 选型 / 计划时：统一流程、阶段确认与就绪门禁以 `SKILL.md` 为准；协作门禁有项目总规则用项目的，否则用 `SKILL.md` 默认节。数据大屏 HTML 原型委托 `bi-dashboard-generator`（先读对方 `SKILL.md`，未安装则停）；转 Vue 交回本 Skill。混合文档由本 Skill 先判定范围。
...
| 时间 | 说明 |
|---|---|
| 2026-09-03 | 文档落盘后强制独立机审（结论进 review/）；人审「下一步」仍是门禁；HTML 走大屏 5.5；代码按 Mx–My 整段审 |
| 2026-09-01 | 取消轻量/完整分流；统一为规格 → 选型 → 数据模型 → 设计契约 → 开发计划 → 实现；无已审核计划不得写业务代码 |
| 2026-09-01 | 文档串行：需求规格 / 选型 / 开发计划一次一份，每份审核后手动确认下一步；未确认问题必须先问，禁止打包成文 |
...
| 2026-09-01 | 增加对话确认卡与文档/实现分阶段 `go`；禁止同一轮既写规格又写代码 |
